...
static void error(const char* msg) { /* report error message */
    ...
    if (do_lint) {
		...
		if (len > WIDTH) {            /* too long record */
			...
			offset = last - WIDTH;
//			for (k = 0; k<WIDTH; k++) buffer[k] = csv_record[k+offset];
//			textline = buffer;
...
//fflush(stderr);
    ...
    if (*ofs=='\0') p.begin_field = &begin_field0;
    else             p.begin_field = &begin_field;
