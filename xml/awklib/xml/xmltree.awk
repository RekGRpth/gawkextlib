...
#------------------------------------------------------------------
# Internal: print attribute markup ' name="value"'
...
# XmlPrintElementStart( nodeindex )  prints '<tag attr="value" ...>'
#------------------------------------------------------------------
function XmlPrintElementStart( node,       k, attr ) {
...
function _Xml_Check( from, pathpattern, valpattern,    k, path ) {
    if (pathpattern=="^$") {
        return valpattern=="" || match(_Xml_CollectValue(from), valpattern)
    } else {
        for (k=from+1; path=_Xml_Path(from, k); k++) {
            if (match(path, pathpattern)) {
                if (valpattern=="" || match(_Xml_CollectValue(k), valpattern)) {
                    return 1
                }
            }
        }
...
#------------------------------------------------------------------
#  Internal: recursively collect the text content of a node
function _Xml_CollectValue( node, yetcollected,      k, child, value ) {
    value = ""
    if (!(node in yetcollected)) {
        yetcollected[node] = node
        if (_XML_[node, "children"]) {
            for (k=1; k<=_XML_[node, "children"]; k++) {
                value = value _Xml_CollectValue( _XML_[node, "child", k], yetcollected )
            }
        } else {
...
#------------------------------------------------------------------
#  XmlGetValue: collect the text content of a set of nodes
#
...
function XmlGetValue( rootnode, path ) {
    ...
    for (Xml_j=1; Xml_j<=Xml_n; Xml_j++) {
        Xml_value = Xml_value _Xml_CollectValue( Xml_nodeset2[Xml_j], Xml_nodeset )
    }
    return Xml_value
}
...
#==================================================================
#     Functions that create the XML tree
#==================================================================

#------------------------------------------------------------------
#  Internal: add a new child node
function Xml_NewChild( node,     child ) {
    child = ++_XML_["last"]
...
#------------------------------------------------------------------
#  Internal: add a new atribute node
function Xml_NewAttribute( node, name, value,     attr ) {
    attr = ++_XML_["last"]
...
#==================================================================
#     Rules that store the XML tree in the _XML_ array
#==================================================================

# Initializes an empty tree and the default output
BEGIN {
    _XML_["last"] = 0
    _Xml_CurrentNode = 0
    _XML_[0] = ""
#    xwopen( "/dev/stdout" )  # default output - NO! already done by xmlwrite
}
